...
neo4j_db = os.getenv("PURPLEPANDA_DB_NAME")
...
if not neo4j_pwd:
    print("Error: no env variable PURPLEPANDA_PWD with the neo4j password")

if neo4j_url and neo4j_pwd:
    if neo4j_db:
        repo = Repository(neo4j_url, password=neo4j_pwd, name=neo4j_db)
    else:
